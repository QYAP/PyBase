...
class Validator():
    ...
    def standard_rule(self):
        ...
        for arg_name, arg_type in list(self.rule.items()):
            # rule-dict 的值必须为一个类或者BaseType及其子类的一个实例
            if not isinstance(arg_type, type) and not issubclass(type(arg_type), BaseType):

                raise Exception("rule-dict value error，rule-dict's value must be a class or instance of BaseType'sub-class!")
            ...
            # 处理rule-dict key键为tupple的情况，并规范化value值
                ...
                raise Exception("rule-dict key error，rule-dict's key format error!")
                ...
                if arg_flag_name[1] in self.args:
                    if arg_type.process_flag == 0:
                        if type(self.args[arg_flag_name[1]]) not in arg_type.field:
                            raise Exception("args type error,type of %s should be %s!" %
                                            (arg_flag_name[1], str(arg_type.field)))
                else:
                    raise Exception("args lack error,lack %s" % arg_flag_name[1])
            # 如果该参数是可缺省参数
    ...
    def check_redundant(self):
        ...
        if self.redundant_or_not:
            if redundant_num > self.redundant_max_num:
                raise Exception("args redundant error,%s more  args were given!" % (redundant_num - self.redundant_max_num))
        else:
            if redundant_num > 0:
                raise Exception("args redundant error,%s more  args were given!" % redundant_num)
